...
AI_PM 是一组运行在 Claude Code 上的技能（Skills），面向产品经理日常工作场景。用户通过探索研究、决策分析、PRD、原型、评审验收、运营复盘六种工作模式进入任务；底层继续复用现有 skills 和 phase/status，支持多项目并行管理与断点续传。项目根目录下有 `AI_PM_教程中心.html`，用浏览器打开可查看所有功能的可视化使用指南。
...
| `/ai-pm explore [任务]` | 探索研究（需求/竞品/访谈/数据/缺口） |
| `/ai-pm decide [任务]` | 决策分析（优先级/战略沙盘/战略求证） |
| `/ai-pm prd [需求或路径]` | PRD 独立一级入口（生成或更新 Markdown PRD） |
| `/ai-pm prototype [项目]` | 原型模式（迭代项目先核当前产品与端别证据） |
| `/ai-pm review [对象]` | 评审验收模式（文档评审或真实实现验收） |
| `/ai-pm operate [任务]` | 运营复盘模式（impact/复盘/周报/知识/分享） |
| `/ai-pm reconcile` | 范围变化后的跨产物只读一致性检查 |
| `/ai-pm impact` | 发布后效果回收，证据不足可继续观察 |
| `/ai-pm retrospective --system` | AI_PM 跨会话系统复盘，不读取/复制 raw |
...
**篇幅指引（字数三档，单源=判断卡 §7.1）**：≤1 万字目标 / 超 1 万至 1.5 万须超因头注释 / >1.5 万停——先可读性精简（安全带：存档+规则丢失复查），仍超走台账外置或拆版本（须用户拍板）；字数以 `scripts/check-prd-word-count.py` 为准。
...
- **一切交付给人读的散文都要过文字关**（2026-09-17 扩大范围，原先只管交互文案）：
  ...
  - 自动查的排除项：`_cloud/` 备份、`_feishu-archive/` 人类正本、**文件头带 `READONLY SNAPSHOT` 的飞书正本快照**（知识库蒸馏产物，别人写的，量它没有可执行意义——2026-09-17 补，涉及 2809 份）、README / CLAUDE.md / SKILL.md、`.claude/` 与 `templates/`（这些是给机器读的配置，不是散文）
  - 判定规则「≥2 项超 P95，或 ≥3 项超 P90」，人类语料实测误报 3.7%
  - ⚠️ **基线缺失时降级成「只跑硬拦词规则」，不做分位判定**：没有人类分布就没有"偏了多少"可言。这里原先想退回一组内置 p90 常量，实测是死代码——空分布让两个 low 方向指标恒定拿满极端度，把每篇文档都判成未过（2026-09-17 Codex 复核实证，fresh clone / CI 必踩）。`--selftest` 有专门回归闸守着，别删
  ...
  - **词表分两档，分界线由实测密度定、不由直觉定**（2026-09-17 用外部集重订）：**硬拦档**（「接住」「路是通的」「总体而言」等 12 词，在 445 万字人类正本里 ≤1 次）命中即判未过；**提示档**（抓手 12 次 / 发挥 10 次 / 整体来看 / 有一定 / 略显 / 心智 / 基本可用）只提示不判死——真人在用，硬拦会误伤
  - ⚠️ **往硬拦档加词前必须先在外部集上数一遍**：原表的「人类语料 0 次」是拿作者本人 82 份 PRD（26 万字）验的，语料太窄；换成 2607 份同事飞书正本（445 万字，未参与建表）复验，19 个词里 12 个没过。**0 次不是因为词罕见，是因为只看了一个人写的东西**
...
> **静默护栏（第一原则，压在所有知识库治理之上）**：任何自动/治理动作都不得打断或刷屏用户正在进行的操作。耗时执行（复核分诊、批量核实）只在用户**显式调用**知识命令时发生；自动路径必须**静默、亚秒级、零额外输出**。⛔ 严禁出现"对话进行中突然长时间跑沉淀、把对话刷上去"。完整原则见 `ai-pm-knowledge/SKILL.md` §运行原则。
