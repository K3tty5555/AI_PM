...
```bash
cd <AI_PM_ROOT>/app/src-tauri && source ~/.cargo/env && cargo build 2>&1 | tail -5
```
...
```bash
cd <USER_HOME>/workplace/AI_PM && git add app/src-tauri/build.rs app/src-tauri/.gitignore && git commit -m "feat: build.rs 编译时复制 skills 到 resources"
```
...
```bash
cd <AI_PM_ROOT>/app/src-tauri && python3 -c "import json; json.load(open('tauri.conf.json')); print('JSON OK')"
```

**Step 3: Commit**

```bash
cd <USER_HOME>/workplace/AI_PM && git add app/src-tauri/tauri.conf.json && git commit -m "feat: tauri.conf.json 配置 bundle resources"
```
...
```bash
cd <AI_PM_ROOT>/app/src-tauri && source ~/.cargo/env && cargo check 2>&1 | tail -5
```
...
```bash
cd <USER_HOME>/workplace/AI_PM && git add app/src-tauri/src/commands/stream.rs && git commit -m "feat: stream.rs 改用内置 resources/skills，CLI 用 output_dir"
```
...
```bash
cd <USER_HOME>/workplace/AI_PM && git add app/src-tauri/src/state.rs app/src-tauri/src/lib.rs && git commit -m "feat: 移除 ai_pm_root — AppState 和 lib.rs 精简"
```
...
```bash
cd <AI_PM_ROOT>/app && npx tsc --noEmit 2>&1 | head -10
```
...
```bash
cd <USER_HOME>/workplace/AI_PM && git add app/src/pages/Settings.tsx && git commit -m "fix: 移除 Settings 硬编码技能路径"
```
...
```bash
ls <AI_PM_ROOT>/app/src-tauri/resources/skills/ | head -10
```
...
```bash
cd <AI_PM_ROOT>/app && source ~/.cargo/env && npm run tauri dev > /tmp/tauri-dev.log 2>&1 &
