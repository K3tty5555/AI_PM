...
```bash
mkdir -p <AI_PM_ROOT>/app/public/fonts
cp <AI_PM_ROOT>/web/app/fonts/GeistVF.woff2 <AI_PM_ROOT>/app/public/fonts/
cp <AI_PM_ROOT>/web/app/fonts/GeistMonoVF.woff2 <AI_PM_ROOT>/app/public/fonts/
```
...
```bash
mkdir -p <AI_PM_ROOT>/app/src/lib
mkdir -p <AI_PM_ROOT>/app/src/components/ui
```
...
```bash
cargo check --manifest-path <AI_PM_ROOT>/app/src-tauri/Cargo.toml 2>&1 | grep -E "error|warning: unused" | head -20
```
...
```bash
cargo check --manifest-path <AI_PM_ROOT>/app/src-tauri/Cargo.toml 2>&1 | grep "^error" | head -20
```
...
```bash
cd <AI_PM_ROOT>/app && npx tsc --noEmit 2>&1 | grep "error TS" | head -20
```
...
```bash
open <AI_PM_ROOT>/app/src-tauri/target/release/bundle/macos/AI\ PM.app
```
...
config.ai_pm_root = '<USER_HOME>/workplace/AI_PM';
