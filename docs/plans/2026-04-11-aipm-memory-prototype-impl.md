...
**Files:**
- Create: `templates/presets/某 K12 教育平台B端.md`
...
**Step 2: 创建 templates/presets/某 K12 教育平台B端.md**
...
> 此文件用于 `/ai-pm new {项目名} --preset=某 K12 教育平台B端` 命令。
...
## 业务域
某 K12 教育平台，覆盖考试、作业、学情分析三条线。
主要产品线：某模块（web-precision-agent）、题目设计（web-pt-dj-front）、
...
## 代码仓路径（供设计指纹提取）
某模块：/Users/xiaowu/workplace/company_CODE/web-precision-agent
题目设计：/Users/xiaowu/workplace/company_CODE/web-pt-dj-front
容器化：/Users/xiaowu/workplace/company_CODE/zx-container-web
设计工作台：/Users/xiaowu/workplace/company_CODE/designer-workbench
```

**Step 3: 验证写入成功**

读取 `templates/presets/某 K12 教育平台B端.md` 确认内容完整，特别是「设计 Token」和「代码仓路径」章节存在。

**Step 4: Commit**

```bash
cd /Users/xiaowu/workplace/AI_PM
git add templates/presets/某 K12 教育平台B端.md
git commit -m "feat(memory): add 某 K12 教育平台B端 preset template"
```
...
1. 执行 `/ai-pm new 考试智能批阅 --preset=某 K12 教育平台B端`
   - 预期：`_memory/L0-identity.md` 被创建并包含某 K12 教育平台B端.md 的内容
...
1. 执行 `/ai-pm prototype --codebase=/Users/xiaowu/workplace/company_CODE/web-precision-agent`
