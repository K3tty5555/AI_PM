...
const RETRO_FILE = "10-retrospective.md"
...
export function RetrospectivePage() {
  ...
  // Load existing retrospective file or trigger AI
  useEffect(() => {
    if (!projectId) return
    let cancelled = false
    ...
    async function loadExisting() {
      try {
        const content = await api.readProjectFile(projectId, RETRO_FILE)
        if (!cancelled) {
          if (content) {
            ...
            if (!startedRef.current) {
              ...
              start([{ role: "user", content: "请基于项目全流程产出物进行项目复盘" }])
            }
          }
        }
      } catch (err) {
        console.error("Failed to load retrospective file:", err)
  ...
  const handleGenerate = useCallback(() => {
    startedRef.current = true
    start([{ role: "user", content: "请基于项目全流程产出物进行项目复盘" }], { excludedContext })
