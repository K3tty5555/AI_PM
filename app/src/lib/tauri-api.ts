...
export interface KnowledgeEntry {
  ...
  confidence?: string
  /** Whether the card was created via auto-capture hook */
...
export interface PrdFileEntry {
  /** Filename relative to 05-prd/ */
  file: string
  /** Display label: "V1.0" / "V1.1" / "未版本" */
  label: string
  /** Sort key: V1.0=100, V1.1=110, V2=200, unrecognized=0 */
  sortKey: number
  /** True if version regex matched */
  recognized: boolean
  /** Parent version label (from _status.json, optional) */
  parent?: string
  /** Custom business label (from _status.json, e.g., "V1.1 搜题场景") */
  customLabel?: string
}

export interface AgentErrorEntry {
  /** Wave/role key, e.g. "wave1_analyst" */
  key: string
...
export interface PrototypeVersionEntry {
  /** Directory path (with trailing slash), e.g., "06-prototype/" or "06-prototype-v1.3/" */
  dir: string
  /** Display label: "默认" / "v1.3" / "preview" */
  label: string
  /** Sort key */
  sortKey: number
  /** True if manifest.json exists (multi-file prototype) */
  hasManifest: boolean
  /** Custom label from _status.json */
  customLabel?: string
}
...
export const api = {
  ...
  cleanupAutoKnowledge: (args: {
    category?: string
    maxConfidence?: "low" | "medium" | "high"
    action: "delete" | "archive"
    ids?: string[]
  }) => safeInvoke<{ processed: number; failed: string[] }>("cleanup_auto_knowledge", { args }),
  ...
  listPrototypeVersions: (projectId: string) =>
    safeInvoke<PrototypeVersionEntry[]>("list_prototype_versions", { projectId }),
  setPrdVersionMeta: (args: {
    projectId: string
    file: string
    label: string | null
    parent: string | null
  }) => safeInvoke<void>("set_prd_version_meta", { args }),
  setPrototypeVersionMeta: (args: {
    projectId: string
    dir: string
    label: string | null
  }) => safeInvoke<void>("set_prototype_version_meta", { args }),
  listMemoryFiles: (projectId: string) =>
    safeInvoke<MemoryFileEntry[]>("list_memory_files", { projectId }),
  ...
  getAgentErrors: (projectId: string) =>
    safeInvoke<AgentErrorEntry[]>("get_agent_errors", { projectId }),
  clearAgentErrors: (projectId: string, keys: string[]) =>
    safeInvoke<void>("clear_agent_errors", { projectId, keys }),
