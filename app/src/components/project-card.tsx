...
import { Trash2, CheckCircle2, RotateCcw, Pencil, FolderOpen, ExternalLink, Star, Check } from "lucide-react"
...
export const ProjectCard = memo(function ProjectCard({
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (e.metaKey || e.ctrlKey) {
      e.preventDefault()
      onToggleSelect?.(project.id, false)
      return
    }
    navigate(`/project/${project.id}/${project.currentPhase}`)
  }
  ...
  const card = (
    <div
      ...
      className={cn(
        ...
        selectionMode && isSelected
          ? "border-[var(--accent-color)] ring-2 ring-[var(--accent-ring)]"
          : "border-[var(--border)]",
        !selectionMode && "hover:shadow-[var(--shadow-md)] hover:-translate-y-[1px]",
    ...
    >
      ...
      {/* Card action buttons -- top-right, revealed on hover (hidden in selection mode) */}
      {!selectionMode && (
      ...
      {/* Project name */}
      <div className={cn("flex items-start gap-2 pr-6 mb-3", selectionMode && "pl-7")}>
        {isEditing ? (
      ...
      {/* Phase badge */}
      <div className={cn(selectionMode && "pl-7")}>
  ...
  // In selection mode, skip context menu to avoid interference
  if (selectionMode) return card

  return <ContextMenu items={contextItems}>{card}</ContextMenu>
})
