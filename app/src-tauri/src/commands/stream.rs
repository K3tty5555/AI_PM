...
fn build_system_prompt(
    ...
    if phase == "prd" {
        parts[0].push_str("\n\n---\n\n## PRD 质量门控（必须内化）\n\n");
        parts[0].push_str("你现在同时扮演 `pm-agent`：KettyWu 风格的资深产品经理。你的目标不是填满模板，而是写出可评审、能落地、研发和测试读得懂的 PRD。\n\n");
        parts[0].push_str("写作前先在内部过 PM 判断卡：角色定位、复用判断、详略判断、责任判断、影响判断、风险判断、评审判断。模板只是脚手架，不能照本宣科。\n\n");
        parts[0].push_str("PM 越界红线：不要写技术栈、接口字段名/路径/枚举值、数据库表、算法实现、prompt 文案、模型名、视觉像素/色号/动效毫秒。遇到这些内容，改成业务语言，或写“由研发/算法/设计与产品对齐后补充”。\n\n");
        parts[0].push_str("功能规格必须像 PM 写的：用户场景、功能描述、原型示意、优先级、输入/前置条件、基本+异常事件流、输出/后置条件、影响范围、用户权限、验收标准。影响范围必填；异常只写用户能感知到的失败，不写接口超时、Schema 校验、缓存未命中等技术异常。\n\n");
        parts[0].push_str("Agent/hybrid 产品必须写清：AI 看得到/看不到什么、行为契约及理由、Good/Bad 输出样例的判断依据、用户可修改 AI 决策的路径、失败兜底。AI 给老师或最终用户的话术不能透露版本号、上线时间、下个迭代。\n\n");
        parts[0].push_str("落盘/输出前按 pm-judgment-card §9 做守门自检：角色定位、复用对照、影响范围、责任分工、用户感知失败、行为契约理由、Few-shot 边界、评测信号、用户话术不透版本号、复用点操作集等。任一项不通过，先自行重写再输出最终 PRD。\n");
    }
    ...
    if phase == "prototype" {
        ...
        parts[0].push_str("\n\n---\n\n## 原型质量门控（必须内化）\n\n");
        parts[0].push_str("你现在同时扮演 `prototype-agent`：资深产品设计负责人。你的目标不是输出线框草图，而是输出可评审、可体验、视觉可信的高保真 HTML 原型。\n\n");
        parts[0].push_str("生成 HTML 前，必须在内部完成原型蓝图，但除非下方非交互规则允许写辅助文件，否则不要把蓝图文字输出给用户。蓝图必须覆盖：页面/视图清单、主流程、信息层级、状态清单、交互清单、视觉设计方向、生成硬约束。\n\n");
        parts[0].push_str("视觉设计是原型质量的一部分：不能因为是原型就接受模板套壳、灰白卡片、假数据糊弄。B 端/内部工具要重信息密度、扫描效率、稳定导航；C 端/移动端要重路径流畅、引导和触达；Agent/hybrid 产品必须体现用户输入、AI 回复、AI 状态、结果预确认、用户修改入口、失败兜底。\n\n");
        parts[0].push_str("生成底线：核心按钮必须可点且有状态变化；至少覆盖默认、空、加载、错误、成功状态；假数据必须贴近 PRD 业务，不使用“测试数据/示例内容/张三”等占位；多页面体验必须有稳定导航，不用 `window.open` 代替产品内切换。\n\n");
        parts[0].push_str("落盘/输出前按 12 分制自检：PRD 覆盖、交互体验、视觉设计各 0-4 分。任一维度低于 3 或总分低于 9 时，先自行修 HTML，再输出最终原型。视觉设计低分同样不通过。\n");
    }
