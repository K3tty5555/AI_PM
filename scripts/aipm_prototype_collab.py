...
const cards=[...document.querySelectorAll('.frame-card')];function persist(){localStorage.setItem(storageKey,JSON.stringify(data));updateSummary()}function updateSummary(){const values=Object.values(data);const passed=values.filter(x=>x.status==='passed').length;const issues=values.filter(x=>x.status==='open').length;const unreviewed=cards.filter(card=>!card.hidden&&(data[card.dataset.frame]?.status||'unreviewed')==='unreviewed');const percent=Math.round(passed/cards.length*100);document.getElementById('progressFill').style.width=percent+'%';document.querySelector('[role=progressbar]').setAttribute('aria-valuenow',percent);document.getElementById('progress').textContent=`${passed}/${cards.length} 已确认 · ${issues} 个有问题`;const next=document.getElementById('nextUnreviewed');if(next){next.disabled=!unreviewed.length;next.textContent=unreviewed.length?`下一条未确认（${unreviewed.length}）`:'已全部检查'}}
