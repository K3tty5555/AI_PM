...
4. **应用风格配置**：
   - 根据 `structure.chapterOrder` 调整章节顺序
   - 根据 `writing.terminology` 使用指定的术语
...
根据 `style-config.json` 中的 `structure` 配置：
...
**应用方式**：
- 按照 `chapterOrder` 的顺序组织章节
- `requiredChapters` 中的章节必须保留，不能省略
- 根据 `tableFrequency` 决定表格使用密度（high/medium/low）
- 控制标题层级不超过 `maxHeadingLevel`

### 2. 术语替换

根据 `style-config.json` 中的 `writing.terminology`：
...
**应用方式**：
- 使用指定的优先级术语（P0/P1/P2 或 高中低）
- 在功能清单中使用对应的优先级描述
- 统一使用配置中的角色术语和行为术语

### 3. 格式适配

根据 `style-config.json` 中的 `formatting`：
...
**应用方式**：
- 功能清单表格使用 `tableFields.functionTable` 指定的字段
- 详细功能设计使用 `tableFields.detailTable` 指定的字段
- 重点内容使用 `emphasisStyle` 指定的标注方式（bold/quote/highlight）
- 流程描述使用 `flowFormat` 指定的格式（text/mermaid/pseudo）

### 4. 内容侧重

根据 `style-config.json` 中的 `contentFocus`：
...
```json
{
  "contentFocus": {
    ...
    "technicalDetail": "light"
  }
}
```

**应用方式**：
- `userStoryWeight`: 控制用户场景描述的篇幅（heavy/medium/light）
- `metricsDetail`: 控制数据指标的详细程度（detailed/standard/brief）
- `exceptionCoverage`: 控制异常处理的覆盖范围（comprehensive/standard/basic）
- `technicalDetail`: 控制技术细节的涉及程度（detailed/moderate/light）
...
- **修订日志**：首次生成时创建 v1.0，后续评审后在原文档直接更新修订日志
...
**不在原文档上直接修改，不创建新文件版本**。保持文档唯一性，通过修订日志追踪变更。

### 修改步骤

1. **读取现有PRD**：读取 `05-PRD-v1.0.md`
2. **在修订日志中增加新记录**：
   ```markdown
   | 2026-03-01 | v1.1 | 评审后修改：修复30个问题（2 Critical + 12 Major + 16 Minor） | 全模块 | AI_PM | 基于评审报告v1 |
   ```
3. **直接在原文档中修改内容**：
   - 修改具体章节内容
   - 新增章节直接插入到合适位置
   - 删除内容用删除线标注（如：~~旧内容~~）
4. **可选生成修改说明**：生成 `08-review-changes.md` 记录详细修改项
5. **不创建备份文件**：不生成 `.bak` 文件，Git历史已足够
...
|------|---------|---------|-------------|------|------|
| 2026-03-01 | v1.0 | 初稿创建 | - | AI_PM | - |
| 2026-03-01 | v1.1 | 评审后修改：修复30个问题（2 Critical + 12 Major + 16 Minor），详见第9章 | 全模块 | AI_PM | 基于评审报告v1 |
```
...
| 编号 | 问题 | 修复内容 | 位置 |
|------|------|---------|------|
| RD-01 | 画布批注技术复杂度 | 增加技术预研计划（M1阶段2周），准备简化版Plan B | 功能清单、里程碑 |
...
```
