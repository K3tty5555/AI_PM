...
4bis. 承重骨架 + doctype 检测：按 pm-agent Mode C 步骤 3bis——读 doctype 标记（`decision_review` 跳过 / `full` 窄检 §一/二/三/六 骨架），**只提示不阻断**，吐机器行 `STRUCTURE_HINT: {missing_skeleton|pass|skipped_decision_review}`、冲突附 `DOCTYPE_WARNING: conflict`（逻辑单一事实源在 pm-agent，driver 只点名、不复制）
...
8. 输出格式：先 ❌ 越界（必修，行号+建议），再 ⚠️ 缺失（建议补·含承重骨架缺失提示 + 机器行 `STRUCTURE_HINT`/`DOCTYPE_WARNING`），再 ⚠️ 概念落地（行号+建议；含 5b 行话三档：确认行话/引文命中/待人判），再 ⚠️ 生造词（术语审计表：术语/新稿次数/历史命中/判断），再 ⚠️ 防御性膨胀（密度 + 逐条收敛建议；含「话题词重述待人工核」），再 ✅ 通过项，最后 🎯 总评（是否可进评审）

补充：决策评审型 PRD 额外按模板八条必答项扫缺失（唯一事实源在 decision-review-template 头部注释：路线含维持现状/三件套/白话示例/影响边界/图啥预答/最小拍板集合/编号防混/§二绑定推荐路线-禁共性框架）。
