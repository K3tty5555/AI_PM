...
4. 检查缺失项（影响范围 / 暂不纳入本期 / 责任分工 / 修订日志合 V1.0 首评基线口径——首评周期修改熔进 V1.0、定稿后再发才 +V1.x，判断卡 §八）—— 复用对照表**不算通用缺失项**，仅功能迁移/接老存量类 PRD 缺它才标（普通迭代不需要，PITFALL-070）
...
7ter. 附录裸编号密度检（程序在 pm-agent Mode C ⓪quinquies·单一事实源在判断卡 §七闸 5，此处不重列）：`grep -onE "附录[A-Z一二三]-?[0-9]+"` 数正文回指频次（forced-artifact），密度高 = 牺牲可读性的偷懒红旗（待对齐值改 PM 口径归属 / 指针改自描述形 / 纯指针格清空）；只查正文回指、附录定义区豁免
...
补充：决策评审型 PRD 额外按模板十条必答项扫缺失（唯一事实源在 decision-review-template 头部注释：①路线含维持现状-前提已拍过则改 P0/P1/P2 批次形态/②三件套/③白话示例-内部熟系统不科普/④影响边界/⑤图啥预答/⑥最小拍板集合/⑦编号防混/⑧§二绑定推荐路线-禁共性框架/⑨先分诊驱动类型再选证据范式-商务竞争驱动＝具名战役+先例+土办法代价、禁推演替代事实/⑩收尾向前看且落档期-§四默认「后续版本规划」而非「主要风险」、风险禁替兄弟部门操心、行动项不占决策点编号）。
