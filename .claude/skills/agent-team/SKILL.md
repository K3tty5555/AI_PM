...
| 代理 | 角色 | 专长领域 | 主要职责 |
|-----|------|---------|---------|
| 🎤 **user-researcher** | 用户研究员 | 用户访谈、需求挖掘 | 访谈执行、行为观察、洞察分析 |
| 🧩 **requirement-synthesizer** | 需求整合师 | 需求提取、整合结构化 | 需求提取、信息整合、优先级排序 |
...
| 现场调研 | Researcher + Synthesizer | 60分钟 | 串行 |
| 多客户调研 | 多Researcher + Synthesizer | 2小时 | 并行 |
...
```bash
# 单客户深度访谈
/agent-team --mode=serial --roles=user-researcher,requirement-synthesizer,pm \
  "后台管理系统-订单模块改进调研"

# 多客户并行调研
/agent-team --mode=parallel \
  --roles=user-researcher,user-researcher,requirement-synthesizer,pm,designer \
  "CRM系统用户体验调研（3个客户）"
```

**执行流程：**
```
🎯 Agent Team 启动 - 现场调研模式
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
🔄 开始执行...
Phase 1: 并行执行3个客户访谈
Phase 2: 需求整合师分析所有访谈记录
Phase 3: 产品经理基于需求清单编写PRD
Phase 4: UI设计师设计交互原型
```
...
├── 11-field-research/           # 现场调研专属目录
│   ├── 00-prep-notes.md         # 用户研究员：准备笔记
│   ├── 01-interview-guide.md    # 用户研究员：访谈提纲
│   ├── 02-interview-notes.md    # 用户研究员：访谈记录
│   ├── 03-interview-summary.md  # 用户研究员：访谈摘要
│   ├── 04-user-persona.md       # 用户研究员：用户画像
│   ├── 05-research-insights.md  # 用户研究员：洞察报告
│   ├── 06-requirement-list.md   # 需求整合师：需求清单
│   ├── 07-requirement-matrix.md # 需求整合师：需求矩阵
│   ├── 08-prd-draft.md          # 需求整合师：PRD草案
│   └── 09-iteration-log.md      # 迭代记录
...
alias ateam="<USER_HOME>/AI_PM/.claude/skills/agent-team/agent-team-cli.sh"
...
- ✅ **用户研究员** (`user-researcher`) - 现场调研、用户访谈、需求挖掘
- ✅ **需求整合师** (`requirement-synthesizer`) - 需求提取、整合结构化
