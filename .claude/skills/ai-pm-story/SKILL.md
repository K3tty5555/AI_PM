...
- 优先级：P0/P1/P2
- 验收标准：**中文三段式（前提 / 操作 / 预期）**，覆盖正常流程、异常流程、边界情况——**禁用 Given/When/Then 英文句式**（2026-07-03 用户拍板；PRD 会以本文档为骨架，英文腔会回灌进 PRD 验收标准）
...
- 正常流程：前提 {前提条件}；操作 {用户操作}；预期 {预期结果}
- 异常流程：前提 {异常前提}；操作 {用户操作}；预期 {错误处理——落到页面状态，用户在哪看到什么}
- 边界情况：前提 {边界条件}；操作 {用户操作}；预期 {预期行为}
