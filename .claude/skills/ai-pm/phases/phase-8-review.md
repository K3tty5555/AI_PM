# Phase 8: 需求评审

**输入**: `05-prd/` 下最新版本 PRD MD + `06-prototype/index.html`（可选）+ `_memory/L2-prd-versions.md`（若存在）
**输出**: `08-review-report-v1.md`

## 参考文档读取（各阶段前置，自动执行）

在执行本阶段任何操作前，扫描 `{project_dir}/05-prd/` 和 `{project_dir}/07-references/` 下的参考文档并载入上下文：

### 1. PDF 文件（视觉读取，保留截图/流程图/原型）

```bash
ls "{project_dir}/05-prd/"*.pdf "{project_dir}/07-references/"*.pdf 2>/dev/null
```

对每个 PDF，渲染为 PNG 图像（已渲染则跳过）：
```bash
python3 .claude/skills/ai-pm/scripts/pdf_to_images.py "{pdf_path}"
# 输出 IMAGES:<dir>:<count> 表示渲染完成，CACHED:<dir>:<count> 表示已有缓存
```

渲染后使用 Read 工具逐页读取 PNG（每次读 2 页），完整浏览全部页面，提取版本摘要追加到 `_memory/L2-prd-versions.md`（不存在则创建）：
- 版本标识：从文件名提取（如 `V1`、`V2`，无法提取则用文件名前 20 字符）
- 摘要：≤30 字描述功能范围
- 关键变化：与上一版相比新增/删除了什么（首版写"初版"）

### 2. DOCX 文件（文本提取，无图片/流程图信息）

```bash
ls "{project_dir}/05-prd/"*.docx 2>/dev/null
```

对每个 DOCX，检查是否存在同名 `.md`（仅替换扩展名）：
- **不存在** → `python3 .claude/skills/ai-pm/scripts/docx_to_md.py "{docx_path}"`
- **已存在** → 跳过

有新转换 MD 时，读取前 200 行提取摘要，追加到 `_memory/L2-prd-versions.md`（格式同上）。

**优先级**：同一文件同时存在 PDF 和 DOCX，以 PDF 为准（视觉信息更完整）。

若两个目录下均无 PDF/DOCX → 静默跳过，继续正常流程。

**注意**：渲染/转换失败不中断主流程，输出 `SKIP:{文件名}:{原因}` 后继续执行。

## 上线前风险预演（pre-mortem · 评审前强制前置）

进入六角色评审**之前**，先做一次上线前风险预演——**这一步不可跳过**。

1. 读取 `references/risk-frameworks.md`，全组假装本期功能已上线并**彻底失败**，倒推"为什么会死"。
2. **强制跑「红线/合规槽位」这一维**（不可省）：当前项目行业包（`pack[行业]`，若已声明）的红线逐条对照本期改动；无包则当面问业务方"这个产品有没有不能碰的红线"。**核心不预设任何具体行业红线。**
3. 其余 8 类维度（值不值 / 会不会用 / 划不划算 / 做不做得出 / 怎么推 / 战略 / 团队）按本期规模详略。
4. 产出风险清单，**喂给下面的六角色评审**当输入：六角色挑"写出来的毛病"，风险预演补"还没写进 PRD 的死法"。

## 执行方式

**评审角色按 product_type 分级**：

读取 `_memory/L1-decisions.md` 中的 `product_type` 字段，决定评审角色：

| product_type | 评审角色 |
|-------------|---------|
| **traditional** | 六角色（产品 / 前端 / 后端 / 测试 / 设计 / 运营）|
| **agent** | 六角色 + **对话体验师** + **AI 决策合理性审查员**（共 8 角色）|
| **hybrid** | 六角色 + **对话体验师** + **AI 决策合理性审查员**（共 8 角色，重点关注 AI 入口与权限边界）|

各角色并行评审，模拟真实评审会议。此阶段为可选，用户可跳过。

→ 详见 `ai-pm-review` 子技能获取完整评审流程和角色定义。

## Agent 专属评审角色

**仅 agent / hybrid 产品启用**。

### 角色 7：对话体验师

**审视视角**：

| # | 检查项 | 评分依据 |
|---|-------|---------|
| 1 | 触发说法是否覆盖真实用户表达？ | A1 意图清单每个意图 ≥ 3 条用户说法、含口语/简称/否定式 |
| 2 | AI 拒绝/澄清话术是否得体？ | A6.3 拒绝话术，避免"我不会""我只是 AI"等反模式 |
| 3 | 多轮对话状态切换是否流畅？ | 跨意图切换、否定回滚、上下文清不清是否有规则 |
| 4 | 错误兜底文案是否有人情味？ | A7 失败模式表对应文案是否冷冰冰、是否给替代方案 |
| 5 | Few-shot 示例的 Good/Bad 对比是否有说服力？ | A6.4 示例 ≥ 3 组、每组说清"为什么好/不好" |

**评分输出**：每项 1-5 分，附整改建议。

### 角色 8：AI 决策合理性审查员

**审视视角**：

| # | 检查项 | 评分依据 |
|---|-------|---------|
| 1 | AI 自主决策清单是否完整？ | A4 决策清单覆盖所有 AI 替用户做的判断（题量/难度/筛选规则等）|
| 2 | 每个决策的"用户能否修改 + 修改路径 + 是否记忆"是否清楚？ | A4.1 决策表四列齐全 |
| 3 | 自主性等级是否定档？是否过激？ | A3 是否声明本期等级 + MVP 等级；教育/医疗/金融场景是否过早上 L3+ |
| 4 | 决策错误的回路是否健全？ | A4.2 修正方式覆盖所有决策点 |
| 5 | HITL 介入点是否合理？ | A7.2 高代价/不可逆动作必须有 Human 确认 |
| 6 | 是否有 Agent 边界声明（不让 AI 做的事）？ | A10 列了 ≥ 3 条不让 AI 做的事 |

**评分输出**：每项 1-5 分，附整改建议；任一项 ≤ 2 分 = 评审打回。

**评审后修改**：评审打回、或用户要按评审反馈修订 PRD/原型时 → 调用 `ai-pm-review-modify` 技能执行修订（命令行用户同样从这里进入，不必只在客户端改）。

## 评审报告命名规则

评审报告按当前 PRD 版本命名，避免覆盖历史版本：

1. 读取 `_memory/L2-prd-versions.md` 最后一个版本条目的版本标识（如 `V1`、`V2`）
2. 输出文件命名为 `08-review-{版本标识小写}.md`（如 `08-review-v2.md`）
3. 若无版本索引（`L2-prd-versions.md` 不存在）→ 降级使用 `08-review-report-v1.md`（原有命名，向后兼容）

## 续评审 · 逐条过问题（continue 进入 phase 8 时触发）

当 `_status.json` 的 `last_phase = review` 且用户请求逐条过问题时，**必须先做预筛，不能直接拿评审报告给用户过**。

### 预筛流程（三步，约 1-2 分钟）

**Step 1：确认版本关系**

```bash
# 找最新评审报告
ls {project_dir}/08-review-*.md | sort | tail -1

# 找当前 PRD
ls {project_dir}/05-prd/*.md | sort | tail -1
```

若评审报告文件名中的 PRD 版本标识（如 `v1.3`）与当前 PRD 版本（如 `v1.2`）不一致 → 在预筛摘要中标注「⚠️ 版本错位：评审基于 {报告版本}，当前 PRD 为 {prd版本}」，但不中断，继续预筛。

**Step 2：逐条交叉比对**（快速扫描当前 PRD，不深读）

对每条评审问题，在当前 PRD 对应章节查找是否已有处理：

| 判定 | 标准 |
|------|------|
| **已解决** | PRD 对应章节有明确文案应对该问题，包括「不存在该场景」类的主动排除 |
| **未解决** | PRD 对应章节无处理，或明显处理不完整 |
| **需确认** | PRD 有部分处理，但与评审问题描述存在偏差，需用户判断 |

**Step 3：输出预筛摘要，用户确认后再开始逐条**

```
预筛结果（共 N 条，基于当前 PRD {版本}）：
✅ 已解决 X 条（跳过）：#4 #11 ...
⚠️  需确认 Z 条：#3 #8 ...
❌ 未解决 Y 条：#1 #2 #5 #6 #7 #9 #10 ...

只过「未解决」和「需确认」共 Y+Z 条，是否开始？
```

### 注意

- 预筛是快速扫描，目的是过滤明显已解决的问题，避免用户重复确认
- 若用户说「都过一遍」或「全过」→ 跳过预筛直接全过
- 预筛不替代正式评审——预筛通过不等于问题真的解决，只是减少重复讨论

## CTA 审计（产品经理角色专属）

评审阶段，产品经理角色额外检查以下内容：

1. **CTA 公式检查** — PRD 中所有行动号召是否符合 `[动作动词] + [用户得到什么] + [紧迫/简单]` 公式
2. **异议覆盖检查** — PRD 是否回应了「太贵/没用/没时间/怕失败」四类异议中至少 2 类
3. 参照 `templates/presets/copywriting-frameworks.md` 中 CTA 公式和异议处理模板
