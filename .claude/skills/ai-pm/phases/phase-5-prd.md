...
| **补充说明** | {复用规约、特殊约束、验收标准}<br>**验收标准格式按复杂度选**：多条件分支 → Mermaid 决策树/状态机；简单后置断言 → 一句中文；需要 QA 执行的测试清单 → 两列表格（操作/预期结果）；**禁用 Given/When/Then 英文句式（2026-07-03 用户拍板绝对禁，已进 §9.0ter 行话黑名单）**<br>**复用规约**：与 V1 X 唯一差异是 Y |
