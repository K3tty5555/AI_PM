...
- **篇幅预算（字数三档，单源=判断卡 §7.1）**：≤1 万字达标；超 1 万至 1.5 万须 `over-budget-reason` 头注释；>1.5 万停、走外置或拆版本。字数以 `scripts/check-prd-word-count.py` 输出为准
...
接迭代需求 / 改动指令后、做任何分析前，先用 2-4 句复述业务理解要点（本期改什么 / 为什么 / 边界在哪 / 不碰什么）请用户确认——治最高频纠正"需求理解错了"。轻量改动（错别字 / 格式 / 单表微调）豁免。pm-agent 路径已内化同款反射（直觉 14），主对话回退路径照本节执行。
...
- **决策评审型**——目标是拿方向 / go-no-go / 在几个方案间拍板（触发词：用户说"决策评审""定方向""先把方案定下来""go/no-go""几个方案选哪个"，或关键节点依赖未定需先决策）。
  → 用 `templates/prd-styles/default/decision-review-template.md` 的 4 节骨架（一·为什么要做 / 二·打算怎么做 / 三·需要决策的内容 / **四·后续版本规划**）。⚠️ §四 默认是「后续版本规划」不是「主要风险」（必答⑩）——只有本次决策本身就押在某个不确定前提上时，才改回「主要风险」。
  ...
  → **十条必答项逐条对模板填（唯一事实源在模板头部注释，此处只留名目）**：路线必含维持现状（前提已拍过则改 P0/P1/P2 批次形态）/ "为什么做"三件套（用户感受列+量化三层+具体落点）/ 黑箱概念白话示例（受众闸：内部熟系统不科普）/ 影响边界 / "图啥"预答 / 当场要拍的最小集合 / 编号防混 / §二绑定推荐路线（禁共性框架）/ 先分诊驱动类型再选证据范式（商务竞争驱动＝具名战役+先例+土办法代价）/ 收尾向前看且落档期（§四默认后续版本规划、风险禁替兄弟部门操心、行动项不占决策点编号）。
  → ⚠️ **落笔前先做两道分诊**：①这次"做不做"真的还能被推翻吗？不能 → 决策点改问"做多少/什么顺序"、路线走批次而非三选一。②驱动是用户诉求还是商务竞争？商务竞争 → 去业务/售前要**具名战役 + 先例 + 当时的土办法**，别用推演凑证据。
  → 方向拍板后，再按完整功能 PRD 流程（步骤 A.1 起）补 §六详细设计。
  → 仍守 PM 边界（pm-judgment-card 越界红线照旧适用；正文用词过判断卡 §9.3 技术行话对照表，引用原话一字不改）。
...
**步骤 A.0.2：输出档 output_profile（云文档增强 gate）⭐ output_profile 契约单一事实源**

**先探测云渲染器（WS-R3，决定要不要问）**：扫 `.claude/skills/*/cloud-renderer.json`（**机器入口只此一条**；manifest schema / 能力字段 / 发现机制的契约见 `references/cloud-renderer-contract.md`）。
- **有满足最低能力的渲染器**（某 manifest `capabilities` 中 `render_and_push` + `callout` + `text_color` 三者全 `true`）→ 才进入下面询问、允许 `cloud_doc_enhanced`。
- **无 / 只有半残**（最低三件没凑齐）→ **不询问、不展示云增强**，直接 `output_profile = markdown` + 一句「未检测到云渲染器插件，配置后可启用云文档增强」。（无渲染器 = 纯 markdown、零感知；别让没装插件的用户对着用不了的云选项发懵。）

判这份 PRD 是否要**写到云文档正本库**（团队云文档系统，决定是否启用云文档增强标记）——**仅探测到可用渲染器时才问**：

- 询问："这份 PRD 要发到云文档正本库吗？（启用云文档增强：关键摘要出高亮框、语义色标记<风险红 / 次要灰 / 核心黄底>；表格排版优化）[Y/n，默认 n]"
- **Y → `cloud_doc_enhanced`** / **n 或默认 / 没提 → `markdown`**。

判完**立即写**（比照 A.0.1 doctype；**不写 L1**——output_profile 是输出模式开关、非设计决策、无 L1 消费者）：
...
**步骤 A.0.3：写前字数预算（Shape Up appetite：预算约束设计，不是写完再裁）**

doctype 判完立即向用户报一句预算（`decision_review` 不设线、跳过本步）：

> 本 PRD 按判断卡 §7.1 字数三档写：目标 ≤1 万字（≈350-400 行）；超 1 万至 1.5 万须在文件头加 `<!-- over-budget-reason: 一句话超因 -->`；>1.5 万停、走台账外置或拆版本（拆版本须你拍板）。多场景合一时预算不放宽——预估要超就现在拆，不写完再裁。

写作中每完成一个 part 对一次 per-part 配额（判断卡 §七闸 3，配额=膨胀定位诊断、非豁免）；交付前跑 `python3 scripts/check-prd-word-count.py "<PRD路径>"` 定档。**外置侧文档约定**（去处与命名单源=判断卡 §7.1）：论证存档 `05-prd/ai-md/…全量版存档-YYYYMMDD精简前.md`（精简安全带①，修订日志留引用）；台账外置 `05-prd/…-对齐清单.md`（正文附录留 5 行内摘要+指针）。
...
| 档 | AI 对齐附录力度 |
|---|---|
| 复杂 Agent / 多模板（说理区体量，1 万字上下）| 完整附录：执行契约 / 边界异常 / 数据来源边界 / 全量验收（判断卡 §7 闸 4 同构）；逼近 1.5 万硬线优先台账外置 |
| 中等功能（数千字档）| 只补关键边界 + 验收，挂正文末 |
| 单功能补丁（一两千字档）| 压成几条行内：「构建差异 / 异常边界 / 待对齐」|
...
| **补充说明** | {复用规约、特殊约束、验收标准}<br>**验收标准格式按复杂度选**：多条件分支 → Mermaid 决策树/状态机；简单后置断言 → 一句中文；需要 QA 执行的测试清单 → 两列表格（操作/预期结果）；**禁用 Given/When/Then 英文句式（2026-07-03 用户拍板绝对禁，已进 §9.3 行话黑名单）**<br>**复用规约**：与 V1 X 唯一差异是 Y |

```

### 单元格内容分类（写每格前先判类型——治"一坨"，判断卡 §七）

填每格前先判这坨是 **异质多维 / 同质并列·步骤 / 限定边界**，按类型选写法（别混挤一格堆成 run-on）：

- **异质多维内容**（报告含哪些块 / 指标范围 / 对象口径，每项不同种类）→ 每项一行、点名（**加粗标签式优先**，`① 名称：值` 编号带名亦可）：
  `**口径**：错误率最高知识点（≥3 班）取前 5`<br>`**代表错题**：≤10 道，跨班按班拆`<br>`**待关注学生**：本知识点错 ≥2 题`
- **同质并列 / 有序步骤**（改动点、流程步）→ 编号：`① {规则/步骤一}<br>② …<br>③ …`
- **口径 / 边界 / 待对齐** → 单列一行：`**口径与边界**：对比年级仅数据列；<60% 标薄弱（§4.1 约定），不替老师判定`
- **异质多维 5+ 项 / 单条 >100 字** → 不留 cell，拆 `##### 小节 + 标准列表`（**同质改动点仍按既有 ≤7 条留 cell**、不受此条约束）
...
- **验收标准选格式**：多分支条件 → Mermaid；简单断言 → 中文一句话；禁用 Given/When/Then 英文句式（绝对禁，PITFALL-031 + 2026-07-03 用户判卷实教）
- **复用对照**：默认**不写**复用对照表，用行内「同 V1 / 复用一期逻辑 / 同步影响：…」表达即可（制卡 V1.5–V1.8 实测都没有）。**仅"功能迁移 / 老系统接新引擎"类**才画 §4.x 复用对照表（4 列：复用对象 / 复用方式 / 本期改动点 / 不改动项，如批改 V3 旧版答题卡接新版AI批改）；普通迭代（修 bug / 调参 / 加开关）画了全是废话（PITFALL-070）
...
✅ **正确**（业务能力描述）：
> 题目检索：按知识点+难度+题型筛题。输入需包含「4 类题库范围（平台库/区本/校本/个人）」信息，由研发与题库团队对齐字段后落到接口文档，本 PRD 不预设。
...
[ ] 复用对照表：仅功能迁移/接老存量类才写，普通迭代没有不算缺失（PITFALL-070）
...
[ ] 评测用接受度信号：不强塞量化指标百分比
[ ] 用户话术不透版本号 / 上线时间
[ ] 篇幅（字数三档，判断卡 §7.1）：跑 `python3 scripts/check-prd-word-count.py "<PRD路径>"`——≤1 万达标；超 1 万至 1.5 万须 `over-budget-reason` 头注释；>1.5 万停、走台账外置 / 拆版本（须用户拍板）。字数在预算内但评审嫌长 → 双层结构（判断卡 §7 闸 4）
