...
5. **6 表格截图覆盖检查**：扫描 PRD 中 `## 六` 或 `## 6` 级别下的所有详细设计表格，检查每个表格是否含 `原型示意` 行；若某表格无 `原型示意` 行且未注明"无界面交互（原因）"（四态协议标准写法，"纯后端"等旧措辞按此归一），记录为审计警告（⚠️）
...
- 注入式原型尤其注意复用现网容器类带来的继承布局问题（判断卡 §10 末）。
...
预检通过后：**把截图写进原型示意 cell**——`[待补原型：xxx] 描述` / 旧 `[xxx原型]` 整段替换为 `![xxx原型](相对路径)<br>描述`（相对 PRD md 所在目录），然后调用 `md2docx.py` 导出 DOCX。
