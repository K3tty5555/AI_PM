...
基础 prompt 格式（参考 ai-pm-prd/SKILL.md 步骤6.2 的结构；此处布局类型由步骤3用户确认值填入）：
...
> 自然语言输入时，将"流程内容"行的括号说明替换为"基于以下描述生成可视化信息图"，并填入用户的原始描述文字，不需要 Mermaid 代码块。
...
- 生成失败 → baoyu-imagine 内部已自动重试 3 次；仍失败则提示用户决定是否再次运行命令
- 网络超时 → 提示检查网络
...
- **导出流程**：SKILL.md 步骤6 自动调用 baoyu-imagine batch 生成所有 Mermaid 流程图，图片引用写入临时副本 `_export_tmp.md` 供导出使用，原始 PRD 保持不变
