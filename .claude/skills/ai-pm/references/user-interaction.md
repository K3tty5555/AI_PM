...
3. **若不指定 `--preset`**：
   - 创建骨架 `_memory/L0-identity.md`，内容为：
     ```
     # {项目名} · 产品定位
     <!-- Phase 1 需求澄清完成后由 ai-pm 自动填写 -->
     ```
   - 创建骨架 `_memory/L1-decisions.md`，内容为：
     ```
     # {项目名} · 关键决策
     <!-- Phase 5 PRD 生成完成后由 ai-pm 自动填写 -->
     ```
   - 后续各 phase 在完成时覆写对应文件的实际内容
...
| 格式 | 读取方式 | 适用场景 |
|------|---------|---------|
| **PDF** | PyMuPDF 渲染为 PNG 逐页视觉读取，保留截图/流程图/原型标注 | 有图片信息的正式 PRD、设计评审稿 |
| MD / TXT | 直接读取文本 | 纯文字需求草稿 |
| DOCX | 转换为 MD 后读取文本，图片信息丢失 | 无 PDF 版本时的备选 |

**推荐提供 PDF**：DOCX 转 MD 会丢失截图、原型图、流程图等关键视觉信息；PDF 通过图像渲染可完整保留。
...
- PDF 渲染结果缓存于 `/tmp/pdf_pages/{文件名}/`，同次会话内重复调用自动命中缓存
