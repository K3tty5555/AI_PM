...
## 评审收口必做（forced-artifact，Stage3-A4 · 2026-07-12 挂点）

**跨模型意见（D3①）**：评审开工先查 `.ai-shared/review-exchange/inbox-codex/` 有无 Codex 意见待并入；收口时若产生给 Codex 的问题，写 `inbox-claude/` 并登记 ledger——不让用户当消息总线。

**外部反馈回灌（D3③）**：研发疑问（截图/群消息）、线下评审结论，逐条录入项目 `08-reviews/feedback-ledger.md`（`| 日期 | 来源 | 反馈 | 处置 | 销账日期 |`）——评审真身在线下，外部反馈是比 AI 评审意见更值钱的输入；PRD 修订前先清这本台账。

评审结束（无论通过与否）**必须执行约定包 §6 live 回写**：本次评审新拍的术语、口径、被推翻的旧约定，逐条写入项目 `_conventions.md` §6（带日期+来源"评审 {日期}"）；**无新增则在 §6 显式写一行「{日期} 评审：本次无新拍约定」**——空着不算做过。这一步不做，评审不算收口。
