---
name: ai-pm-review
description: 需求评审技能。模拟真实评审会议，九大专家角色从专业角度评审 PRD 和原型，输出评审意见和修改建议。支持多轮迭代。
argument-hint: "[项目目录路径 | --round=N]"
allowed-tools: Read Write Edit Bash(mkdir) Bash(ls) Agent
---

# 需求评审

## 输入

- 主要：`{项目目录}/05-prd/05-PRD-v1.0.md`（PRD）
- 参考：`{项目目录}/06-prototype/index.html`（原型，如有）
- 参考：`{项目目录}/02-analysis-report.md`、`04-user-stories.md`

## 输出

`{项目目录}/08-review-report-v{N}.md`（评审报告）→ 更新 PRD

## 九大评审角色

| 组别 | 角色 | 评审重点 |
|------|------|---------|
| 组1（技术） | 研发总监 | 技术可行性、工期、开发工作量 |
| 组1（技术） | 资深架构师 | 系统架构、扩展性、性能、数据模型 |
| 组1（技术） | QA 总监 | 可测试性、验收标准、边界情况 |
| 组2（产品） | 产品总监 | 需求完整性、用户价值、优先级 |
| 组2（产品） | 设计总监 | 用户体验、交互流程、设计一致性 |
| 组2（产品） | 运营增长官 | 冷启动、运营工具、数据埋点 |
| 组3（合规） | 项目经理 | 排期合理性、里程碑、风险管理 |
| 组3（合规） | 安全合规官 | 数据安全、权限控制、合规要求 |
| 组3（合规） | 数据分析师 | 指标体系、埋点方案、数据质量 |

## 执行步骤

### 步骤1：读取评审材料

读取 PRD、原型、需求分析、用户故事，确定评审轮次（`--round` 参数，默认第1轮）。

### 步骤2：3 组 Subagent 并行评审

调用 `Agent` 工具，同时派发 3 个 subagent：

**Subagent 1 — 技术视角（研发总监 + 架构师 + QA 总监）**
- 系统提示：同时扮演研发总监、架构师、QA 总监三个角色，不与用户交互
- 任务：读取 PRD 和原型，输出三个角色各自的评审意见
- 输出至：`/tmp/review_tech.md`

**Subagent 2 — 产品视角（产品总监 + 设计总监 + 运营增长官）**
- 系统提示：同时扮演产品总监、设计总监、运营增长官三个角色，不与用户交互
- 任务：读取 PRD 和原型，输出三个角色各自的评审意见
- 输出至：`/tmp/review_product.md`

**Subagent 3 — 合规视角（项目经理 + 安全合规官 + 数据分析师）**
- 系统提示：同时扮演项目经理、安全合规官、数据分析师三个角色，不与用户交互
- 任务：读取 PRD 和原型，输出三个角色各自的评审意见
- 输出至：`/tmp/review_compliance.md`

主线程等待 3 个 subagent 完成后：合并结果，去重（多角色提及同一问题则合并并注明来源），按 Critical → Major → Minor 排序。

### 步骤3：生成评审报告

生成 `08-review-report-v{N}.md`，输出问题汇总和修改建议。

### 步骤4：用户确认修改策略

```
评审完成！共发现 {N} 个问题

Critical（必须修改）：{n} 个
Major（建议修改）：{n} 个
Minor（可选优化）：{n} 个

请选择修改策略：
- "全部修改" → 修改所有问题
- "核心修改" → Critical + Major（推荐）
- "最小修改" → 仅 Critical
- "指定修改" → 告诉我具体修改哪些
```

### 步骤5：执行 PRD 修改

**修改原则：不创建新文件，直接在 `05-prd/05-PRD-v1.0.md` 中修改，修订日志追加新记录。**

```markdown
| {日期} | v1.{N} | 评审后修改：修复{N}个问题（{n} Critical + {n} Major） | 全模块 | AI_PM | 基于评审报告v{N} |
```

### 步骤6：询问是否再次评审

```
修改完成。Critical 问题：{n}/{n} 已解决。
是否进行下一轮评审？回复"再次评审"或"确认完成"。
```

## 问题格式

每个问题按以下结构输出：

```markdown
#### 问题 {编号}: {标题}
- **严重程度**: Critical / Major / Minor
- **评审角色**: {角色名}
- **涉及模块**: {模块}
- **问题描述**: {详细描述}
- **修改建议**: {具体做法}
- **参考依据**: {依据，如法规/规范/最佳实践}
```

**严重程度：**
- Critical：影响核心功能或存在重大风险，必须修改，阻塞发布
- Major：影响体验或存在明显缺陷，建议修改
- Minor：细节优化，可选修改

## 输出格式

```markdown
# 需求评审报告 - 第{N}轮

## 评审概览
- 评审时间：{日期}
- 评审对象：{产品名} PRD v{X.X}
- 评审结论：{通过 / 有条件通过 / 需修改后再评审}

## 评审统计
| 角色 | Critical | Major | Minor | 总计 |
|------|----------|-------|-------|------|
{每个角色一行}
| 总计 | {n} | {n} | {n} | {n} |

## Critical 问题（必须修改）
{按问题格式逐一列出}

## Major 问题（建议修改）
{按问题格式逐一列出}

## Minor 问题（可选优化）
{按问题格式逐一列出}

## 评审结论
- [ ] 通过 — 无需修改或仅 Minor，可进入开发
- [ ] 有条件通过 — 修改 Major 后可进入开发
- [x] 需修改后再评审 — 修改 Critical/Major 后进行下一轮
- [ ] 不通过 — 存在重大缺陷，需重新设计
```

## 终轮通过标准

- Critical 问题全部解决
- Major 问题解决率 >= 80%
- 未新引入 Critical 问题
- 安全合规问题必须全部解决
