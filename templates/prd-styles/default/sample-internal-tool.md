...
| **需求描述（异常事件流）** | 异常1：序列号已存在 → 保存时提示"该序列号已登记在资产 {编号} 中，请核实"，不允许保存<br>异常2：批量导入数据校验不通过 → 逐行标记错误原因，用户修改后重新上传<br>异常3：打印机未连接 → 提示"未检测到打印机，请检查连接"，支持导出标签 PDF 手动打印 |
...
| **需求描述（异常事件流）** | 异常1：申请的设备在审批期间被其他申请抢先占用 → 审批时提示"该设备已被借出"，自动推荐同类型闲置设备<br>异常2：借用逾期 3 天未归还 → 系统通知借用人直属主管，同时在工作台高亮显示逾期记录<br>异常3：归还时设备有损坏 → 管理员标记为"有损"，系统自动创建维修工单 |
...
| **需求描述（异常事件流）** | 异常1：维保已过期仍未续保 → 资产详情页显示红色"已过保"标签，工作台置顶高亮展示<br>异常2：录入的维保结束日期早于开始日期 → 表单校验阻止保存，提示"结束日期不能早于开始日期"<br>异常3：邮件发送失败 → 回退到站内信保证送达，运维日志记录邮件发送失败原因 |
