...
| **需求描述（异常事件流）** | 异常1：批量导入时文件格式不正确 → 提示"文件格式不支持，请使用系统模板"并中止导入<br>异常2：导入数据中手机号重复 → 标记重复行，用户选择跳过或覆盖<br>异常3：分配目标销售已离职 → 提示"该销售已停用，请选择其他人" |
...
| **需求描述（异常事件流）** | 异常1：拖拽卡片跨阶段（如从"需求确认"直接到"合同签署"） → 提示"不支持跨阶段推进，请按顺序逐步推进"<br>异常2：拖拽到"已完结"列 → 弹出赢单/输单选择，输单时必须填写输单原因<br>异常3：看板中商机超过 200 条 → 仅展示前 200 条并提示"商机较多，建议使用筛选缩小范围" |
...
| **需求描述（异常事件流）** | 异常1：跟进日期已过 2 天仍未填写跟进记录 → 系统向销售主管推送"跟进逾期"提醒<br>异常2：第三方通知渠道推送失败 → 回退到站内信确保送达，后台记录推送失败日志<br>异常3：同一天有超过 10 个客户待跟进 → 日历页面折叠显示，点击展开完整列表 |
