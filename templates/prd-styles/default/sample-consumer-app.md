<!-- doctype: full -->
> 📎 **样本说明**：本文件为「全员评审 · 功能 PRD」风格样例（虚构产品），示意章节结构与用词风格。部分 2026-06 后约定（`doctype` 机读字段 / 影响范围 / 暂不纳入本期 / 复用对照表）以 `feishu-template.md` + `phase-5-prd.md` 流程为准；文中色值/数据均为示例值，按产品实际替换。⚠️ **另有四类旧示例勿模仿**（2026-07-03 自查标注）：①旧 `[xxx原型]` 占位（现行=四态协议，`![](path)<br>描述`）②动画毫秒/加速度阈值等视觉与实现参数（越界红线：不进 PRD，交设计/研发）③技术栈选型（Swift/Kotlin 等不进 PRD）④埋点参数命名（PRD 只写事件与目的，参数名由研发定）。
