...
| **需求描述（异常事件流）** | 异常1：附近 1km 内无可推荐餐厅 → 自动扩大范围至 3km 并提示"帮你看了更远一点"<br>异常2：推荐接口超时（> 3 秒） → 使用本地缓存的上一次推荐池随机抽取<br>异常3：用户连续换了 10 家都不满意 → 弹出"有点挑剔哦"趣味提示，引导进入筛选模式 |
...
| **需求描述（异常事件流）** | 异常1：用户跳过初始问卷 → 使用当前位置热门餐厅作为冷启动推荐<br>异常2：用户忌口标记了大量食材（超过 10 种） → 提示"忌口太多可能影响推荐效果"但不阻止<br>异常3：偏好模型长期未更新（30 天无行为数据） → 下次打开时温和提示"口味有变化吗？更新一下偏好" |
...
| **需求描述（异常事件流）** | 异常1：超过 30 分钟无人加入房间 → 提示发起人"还没人来，要不自己摇一个？"<br>异常2：投票环节有人迟迟不投票 → 5 分钟后提示发起人可"截止投票"<br>异常3：餐厅投票出现平局 → 自动弹出趣味摇骰子界面，随机选定一家 |
